...
public class UCCCodingCompetition21P3_LongPizza {
	...
	public static void main(String[] args) throws Exception {
		...
		int lengthOfPizza = sc.nextInt();
		int startEating = sc.nextInt();
		int endEating = sc.nextInt();
		int numberOfTasks = sc.nextInt();
		int[] differenceArray = new int[lengthOfPizza];
		int[] prefixSumArray = new int[endEating + 1];
		for (int i = 0; i < numberOfTasks; i++) {
			int startTime = sc.nextInt();
			int endTime = sc.nextInt();
			differenceArray[startTime - 1]++;
			if (endTime < lengthOfPizza) {
				differenceArray[endTime]--;
			}
		}
		long amountOfCheese = 0;
		for (int i = 0; i < endEating; i++) {
			prefixSumArray[i + 1] = prefixSumArray[i] + differenceArray[i];
		}
		for (int i = startEating; i < endEating + 1; i++) {
			amountOfCheese += prefixSumArray[i];
		}
