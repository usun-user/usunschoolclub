...
public class GW_ImposterFriends {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		int[] validFriends = new int[numberOfFriends];
		for (int i = 0; i < numberOfFriends; i++) {
			validFriends[i] = sc.nextInt();
		}
		StringBuilder output = new StringBuilder();
		String lineBreak = System.getProperty("line.separator", "\n");
		int length = validFriends.length - 1; // is this faster?
		for (int i = 0; i < numberOfOthers; i++) {
			...
			while (high >= low) {
				...
				int pointerValue = validFriends[pointer]; // is this faster?
				if (target < pointerValue) { //
					high = pointer - 1;
				} else if (target > pointerValue) { //
					low = pointer + 1;
				} else {
					output.append("yes");
					break;
